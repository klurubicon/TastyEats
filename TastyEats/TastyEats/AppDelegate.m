...
#import "TEWelcomeViewController.h"
...
@property (nonatomic, strong) TEHomeViewController *homeViewController;
@property (nonatomic, strong) TEActivityFeedViewController *activityViewController;
@property (nonatomic, strong) TEWelcomeViewController *welcomeViewController;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [Parse setApplicationId:@"03odLEobIGVCkKUXOlOmmQuY3twbIc88WYHl8c9a" clientKey:@"pc5C6xGljjlh44nfkNWZBKaqszSJR2LkNmkU0YM9"];
    ...
    self.welcomeViewController = [[TEWelcomeViewController alloc] init];
...
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo {
    [[NSNotificationCenter defaultCenter] postNotificationName:TEAppDelegateApplicationDidReceiveRemoteNotification object:nil userInfo:userInfo];
    ...
    if ([PFUser currentUser]) {
        if ([self.tabBarController viewControllers].count > TEActivityTabBarItemIndex) {
            UITabBarItem *tabBarItem = [[self.tabBarController.viewControllers objectAtIndex:TEActivityTabBarItemIndex] tabBarItem];
...
- (BOOL)tabBarController:(UITabBarController *)aTabBarController shouldSelectViewController:(UIViewController *)viewController {
    ...
    return ![viewController isEqual:aTabBarController.viewControllers[TEEmptyTabBarItemIndex]];
}
...
- (void)presentTabBarController {    
    self.tabBarController = [[TETabBarController alloc] init];
    self.homeViewController = [[TEHomeViewController alloc] initWithStyle:UITableViewStylePlain];
    [self.homeViewController setFirstLaunch:firstLaunch];
    self.activityViewController = [[TEActivityFeedViewController alloc] initWithStyle:UITableViewStylePlain];
...
- (void)logOut {
    // clear cache
    [[TECache sharedCache] clear];

    // clear NSUserDefaults
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:kTEUserDefaultsCacheFacebookFriendsKey];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:kTEUserDefaultsActivityFeedViewControllerLastRefreshKey];
    [[NSUserDefaults standardUserDefaults] synchronize];

    // Unsubscribe from push notifications by removing the user association from the current installation.
    [[PFInstallation currentInstallation] removeObjectForKey:kTEInstallationUserKey];
    [[PFInstallation currentInstallation] saveInBackground];
...
// Set up appearance parameters to achieve TastyEats's custom look and feel
- (void)setupAppearance {
...
- (void)handlePush:(NSDictionary *)launchOptions {
    ...
    if (remoteNotificationPayload) {
        [[NSNotificationCenter defaultCenter] postNotificationName:TEAppDelegateApplicationDidReceiveRemoteNotification object:nil userInfo:remoteNotificationPayload];
        ...
        NSString *photoObjectId = [remoteNotificationPayload objectForKey:kTEPushPayloadPhotoObjectIdKey];
        if (photoObjectId && photoObjectId.length > 0) {
            [self shouldNavigateToPhoto:[PFObject objectWithoutDataWithClassName:kTEPhotoClassKey objectId:photoObjectId]];
            return;
        }
        
        // If the push notification payload references a user, we will attempt to push their profile into view
        NSString *fromObjectId = [remoteNotificationPayload objectForKey:kTEPushPayloadFromUserObjectIdKey];
        if (fromObjectId && fromObjectId.length > 0) {
            PFQuery *query = [PFUser query];
            query.cachePolicy = kPFCachePolicyCacheElseNetwork;
            [query getObjectInBackgroundWithId:fromObjectId block:^(PFObject *user, NSError *error) {
                if (!error) {
                    UINavigationController *homeNavigationController = self.tabBarController.viewControllers[TEHomeTabBarItemIndex];
...
- (BOOL)handleActionURL:(NSURL *)url {
    if ([[url host] isEqualToString:kTELaunchURLHostTakePicture]) {
        if ([PFUser currentUser]) {
...
- (void)shouldNavigateToPhoto:(PFObject *)targetPhoto {
    ...
    [targetPhoto fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        if (!error) {
            UINavigationController *homeNavigationController = [[self.tabBarController viewControllers] objectAtIndex:TEHomeTabBarItemIndex];
            [self.tabBarController setSelectedViewController:homeNavigationController];
            
            TEPhotoDetailsViewController *detailViewController = [[TEPhotoDetailsViewController alloc] initWithPhoto:object];
