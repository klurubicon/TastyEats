...
@interface TEPhotoDetailsHeaderView ()
...
@implementation TEPhotoDetailsHeaderView
...
- (id)initWithFrame:(CGRect)frame photo:(PFObject*)aPhoto {
    ...
    if (self) {
        ...
        self.photo = aPhoto;
        self.photographer = [self.photo objectForKey:kTEPhotoUserKey];
        self.likeUsers = nil;
...
#pragma mark - TEPhotoDetailsHeaderView
...
- (void)setLikeUsers:(NSMutableArray *)anArray {
    likeUsers = [anArray sortedArrayUsingComparator:^NSComparisonResult(PFUser *liker1, PFUser *liker2) {
        NSString *displayName1 = [liker1 objectForKey:kTEUserDisplayNameKey];
        NSString *displayName2 = [liker2 objectForKey:kTEUserDisplayNameKey];
    ...
    for (TEProfileImageView *image in currentLikeAvatars) {
    ...
    for (i = 0; i < numOfPics; i++) {
        TEProfileImageView *profilePic = [[TEProfileImageView alloc] init];
        ...
        if ([TEUtility userHasProfilePictures:[self.likeUsers objectAtIndex:i]]) {
            [profilePic setFile:[[self.likeUsers objectAtIndex:i] objectForKey:kTEUserProfilePicSmallKey]];
        } else {
            [profilePic setImage:[TEUtility defaultProfilePicture]];
        }
...
- (void)reloadLikeBar {
    self.likeUsers = [[TECache sharedCache] likersForPhoto:self.photo];
    [self setLikeButtonState:[[TECache sharedCache] isPhotoLikedByCurrentUser:self.photo]];
...
- (void)createView {
    ...
    PFFile *imageFile = [self.photo objectForKey:kTEPhotoPictureKey];
    ...
    /* top of header view with name and avatar */
    ...
    [self.photographer fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        // Create avatar view
        TEProfileImageView *avatarImageView = [[TEProfileImageView alloc] initWithFrame:CGRectMake(avatarImageX, avatarImageY, avatarImageDim, avatarImageDim)];

        if ([TEUtility userHasProfilePictures:self.photographer]) {
            [avatarImageView setFile:[self.photographer objectForKey:kTEUserProfilePicSmallKey]];
        } else {
            [avatarImageView setImage:[TEUtility defaultProfilePicture]];
        }
        ...
        // Create name label
        NSString *nameString = [self.photographer objectForKey:kTEUserDisplayNameKey];
        ...
        // resize the button to fit the user's name
        //avoid having a huge touch area
    ...
    /* bottom section fo the header view; the likes */
...
- (void)didTapLikePhotoButtonAction:(UIButton *)button {
    ...
    if (liked) {
        [[TECache sharedCache] incrementLikerCountForPhoto:self.photo];
        [newLikeUsersSet addObject:[PFUser currentUser]];
    } else {
        [[TECache sharedCache] decrementLikerCountForPhoto:self.photo];
    }
    
    [[TECache sharedCache] setPhotoIsLikedByCurrentUser:self.photo liked:liked];

    [self setLikeUsers:[newLikeUsersSet allObjects]];

    if (liked) {
        [TEUtility likePhotoInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
        ...
        [TEUtility unlikePhotoInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
                [button addTarget:self action:@selector(didTapLikePhotoButtonAction:) forControlEvents:UIControlEventTouchUpInside];
                [self setLikeUsers:originalLikeUsersArray];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:TEPhotoDetailsViewControllerUserLikedUnlikedPhotoNotification object:self.photo userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:liked] forKey:TEPhotoDetailsViewControllerUserLikedUnlikedPhotoNotificationUserInfoLikedKey]];
}
