...
@interface TEActivityCell ()

/*! Private view components */
@property (nonatomic, strong) TEProfileImageView *activityImageView;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        horizontalTextSpace = [TEActivityCell horizontalTextSpaceForInsetWidth:0];
        ...
        self.activityImageView = [[TEProfileImageView alloc] init];
...
- (void)setActivity:(PFObject *)activity {
    ...
    _activity = activity;
    if ([[activity objectForKey:kTEActivityTypeKey] isEqualToString:kTEActivityTypeFollow] || [[activity objectForKey:kTEActivityTypeKey] isEqualToString:kTEActivityTypeJoined]) {
        [self setActivityImageFile:nil];
    } else {
        [self setActivityImageFile:(PFFile*)[[activity objectForKey:kTEActivityPhotoKey] objectForKey:kTEPhotoThumbnailKey]];
    }
    
    NSString *activityString = [TEActivityFeedViewController stringForActivityType:(NSString*)[activity objectForKey:kTEActivityTypeKey]];
    self.user = [activity objectForKey:kTEActivityFromUserKey];
    ...
    if (self.user && [self.user objectForKey:kTEUserDisplayNameKey] && [[self.user objectForKey:kTEUserDisplayNameKey] length] > 0) {
        nameString = [self.user objectForKey:kTEUserDisplayNameKey];
    }
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                     ...
                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                        context:nil].size;
        NSString *paddedString = [TEBaseTextCell padString:activityString withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [TEActivityCell horizontalTextSpaceForInsetWidth:insetWidth];
}
...
- (id<TEActivityCellDelegate>)delegate {
    return (id<TEActivityCellDelegate>)_delegate;
}

- (void)setDelegate:(id<TEActivityCellDelegate>)delegate {
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    CGSize nameSize = [name boundingRectWithSize:CGSizeMake(200.0f, CGFLOAT_MAX)
                                                 ...
                                                 attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                    context:nil].size;
    NSString *paddedString = [TEBaseTextCell padString:content withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [TEActivityCell horizontalTextSpaceForInsetWidth:cellInset];
