...
@interface TEWelcomeViewController() {
...
@implementation TEWelcomeViewController
...
#pragma mark - TEWelcomeViewController
...
- (void)presentLoginViewController:(BOOL)animated {
    ...
    TELogInViewController *loginViewController = [[TELogInViewController alloc] init];
    loginViewController.delegate = self;
    [self presentViewController:loginViewController animated:animated completion:nil];
}


#pragma mark - TELoginViewControllerDelegate
...
- (void)refreshCurrentUserCallbackWithResult:(PFObject *)refreshedObject error:(NSError *)error {
    ...
    NSString *facebookId = [currentParseUser objectForKey:kTEUserFacebookIDKey];
    if (!facebookId || ![facebookId length]) {
        // set the parse user's FBID
        [currentParseUser setObject:[FBSDKAccessToken currentAccessToken].userID forKey:kTEUserFacebookIDKey];
    }
    
    if (![TEUtility userHasValidFacebookData:currentParseUser]) {
        NSLog(@"User does not have valid facebook ID. PFUser's FBID: %@, FBSessions FBID: %@. logout", [currentParseUser objectForKey:kTEUserFacebookIDKey], [FBSDKAccessToken currentAccessToken].userID);
        [(AppDelegate *)[[UIApplication sharedApplication] delegate] logOut];
        return;
    }
        ...
        if ([currentAccessToken hasGranted:@"public_profile"]) {
            ...
            [connection addRequest:[[FBSDKGraphRequest alloc] initWithGraphPath:@"me" parameters:@{ @"fields" : @"name" }] completionHandler:^(FBSDKGraphRequestConnection *connection, id result, NSError *error) {
                ...
                if (facebookName && [facebookName length] != 0) {
                    [currentParseUser setObject:facebookName forKey:kTEUserDisplayNameKey];
                }
            ...
            [connection addRequest:[[FBSDKGraphRequest alloc] initWithGraphPath:@"me" parameters:@{@"fields": @"picture.width(500).height(500)"}] completionHandler:^(FBSDKGraphRequestConnection *connection, id result, NSError *error) {
                if (!error) {
                    ...
                    [TEUtility processFacebookProfilePictureData:profilePictureData];
                }
                [self processedFacebookResponse];
            }];
            if ([currentAccessToken hasGranted:@"user_friends"]) {
                ...
                [connection addRequest:[[FBSDKGraphRequest alloc] initWithGraphPath:@"/me/friends" parameters:@{ @"fields": @"id,name,first_name,last_name" }] completionHandler:^(FBSDKGraphRequestConnection *connection, id result, NSError *error) {
                    ...
                    if (error) {
                        ...
                        // cache friend data
                        [[TECache sharedCache] setFacebookFriends:facebookIds];
                        
                        if ([currentParseUser objectForKey:kTEUserFacebookFriendsKey]) {
                            [currentParseUser removeObjectForKey:kTEUserFacebookFriendsKey];
                        }
                        if ([currentParseUser objectForKey:kTEUserAlreadyAutoFollowedFacebookFriendsKey]) {
            ...
            [[TECache sharedCache] clear];
            [currentParseUser setObject:@"Someone" forKey:kTEUserDisplayNameKey];
            _expectedFacebookResponseCount++;
            [self processedFacebookResponse];
        }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    [TEUtility processFacebookProfilePictureData:_profilePicData];
}
