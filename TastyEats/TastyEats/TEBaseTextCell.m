...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        horizontalTextSpace =  [TEBaseTextCell horizontalTextSpaceForInsetWidth:cellInsetWidth];
...
/* delegate that a user image or name was tapped */
...
#pragma mark - TEBaseTextCell

/* get the height for a cell if it had the given name and content */
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content {
    return [TEBaseTextCell heightForCellWithName:name contentString:content cellInsetWidth:0];
}

/* get the height for a cell if it had the given name, content and horizontal inset */
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    ...
    NSString *paddedString = [TEBaseTextCell padString:content withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [TEBaseTextCell horizontalTextSpaceForInsetWidth:cellInset];
...
/* obtain the horizontal space left for name and content after taking the inset and image in consideration */
+ (CGFloat)horizontalTextSpaceForInsetWidth:(CGFloat)insetWidth {
    return (320-(insetWidth*2)) - (horiBorderSpacing+avatarDim+horiElemSpacing+horiBorderSpacing);
}

/*  pad a string with spaces to a given beginning offset */
...
- (void)setUser:(PFUser *)aUser {
    ...
    [self.nameButton setTitle:[self.user objectForKey:kTEUserDisplayNameKey] forState:UIControlStateNormal];
    [self.nameButton setTitle:[self.user objectForKey:kTEUserDisplayNameKey] forState:UIControlStateHighlighted];
...
- (void)setContentText:(NSString *)contentString {
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                     ...
                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                        context:nil].size;
        NSString *paddedString = [TEBaseTextCell padString:contentString withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [TEBaseTextCell horizontalTextSpaceForInsetWidth:insetWidth];
    [self setNeedsDisplay];
}

/* in order to allow inheritance, we add run-time checks. */
- (id<TEBaseTextCellDelegate>)delegate {
    return (id<TEBaseTextCellDelegate>)delegate;
}

- (void)setDelegate:(id<TEBaseTextCellDelegate>)aDelegate {
