...
@implementation TEPhotoHeaderView
...
- (id)initWithFrame:(CGRect)frame buttons:(TEPhotoHeaderButtons)otherButtons {
    ...
    if (self) {
        [TEPhotoHeaderView validateButtons:otherButtons];
        ...
        if (self.buttons & TEPhotoHeaderButtonsComment) {
            // comments button
        ...
        if (self.buttons & TEPhotoHeaderButtonsLike) {
            // like button
        ...
        if (self.buttons & TEPhotoHeaderButtonsUser) {
...
#pragma mark - TEPhotoHeaderView

- (void)setPhoto:(PFObject *)aPhoto {
    photo = aPhoto;

    // user's pic
    PFUser *user = [self.photo objectForKey:kTEPhotoUserKey];
    if ([TEUtility userHasProfilePictures:user]) {
    ...
    NSString *authorName = [user objectForKey:kTEUserDisplayNameKey];
...
+ (void)validateButtons:(TEPhotoHeaderButtons)buttons {
    if (buttons == TEPhotoHeaderButtonsNone) {
        [NSException raise:NSInvalidArgumentException format:@"Buttons must be set before initializing TEPhotoHeaderView."];
    }
}

- (void)didTapUserButtonAction:(UIButton *)sender {
    if (delegate && [delegate respondsToSelector:@selector(photoHeaderView:didTapUserButton:user:)]) {
        [delegate photoHeaderView:self didTapUserButton:sender user:[self.photo objectForKey:kTEPhotoUserKey]];
    }
}
